  ...
  switch(direction) {
    ...
    case 'left':
      if (currentCellIdx > 0) {
        ...
        // 행 시작에서 왼쪽 이동: 윗 행 마지막 셀로
        newRow = rows[currentRowIdx - 1];
        newCell = newRow.querySelectorAll('td[contenteditable]').item(-1);
      }
      break;
    case 'right':
      if (currentCellIdx < cells.length - 1) {
        ...
        // 행 끝에서 오른쪽 이동: 아래 행 첫 셀로
        newRow = rows[currentRowIdx + 1];
        newCell = newRow.querySelectorAll('td[contenteditable]').item(0);
      }
      break;
  }

  // 실제 포커스 이동
  if (newCell !== appState.currentCell.cell) {
